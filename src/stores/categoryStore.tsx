...
const categoryStore = create((set) => ({  
  ...
  fetchCategories: async () => {  
    set({ loading: true });  
    try {  
        const response = await axios.get('http://localhost:8000/api/product-category-firsts', {
          params: {
            limit: 9, 
          },
        });
  ...
  createCategories: async (category) => {
    try {
        ...
        let resp = await fetch(`http://127.0.0.1:8000/api/product-category-firsts`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-TOKEN": csrfToken, // Menambahkan token CSRF
            },
            body: JSON.stringify(category), // Mengubah objek produk menjadi string JSON
        });

        // Memeriksa apakah permintaan berhasil
        if (resp.status == 200) {
            const data = await resp.json(); // Mengambil data dari respons
            set((state) => ({ categories: [...state.categories, data] })); // Menambahkan produk ke state
